...
class CNN_RNN(nn.Module):
  def __init__(self, num_classes, n_frames, n_shift, device):
    ...
    self.convx = nn.Conv2d(1, 16, kernel_size=9, stride=1, padding=2)
    self.conv1 = nn.Conv2d(16, 32, kernel_size=9, stride=1, padding=2)
    self.bn1= nn.BatchNorm2d(16)
    self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=1, padding=2)
    self.dropoutCNN = nn.Dropout2d(p=0.3)
    self.bn2= nn.BatchNorm2d(16)
    self.gru = nn.GRUCell(input_size=64*10*5, hidden_size=8*10*5)
  ...
  def forward(self, x):
    ...
    for locucion in locuciones:
      ...
      for ventana in ventanas:
        ...
        ventana = ventana.unsqueeze(0)
        y = F.max_pool2d((F.leaky_relu(self.convx(ventana))), kernel_size=3, stride=3, padding=0)
        y = F.max_pool2d((F.leaky_relu(self.conv1(y))), kernel_size=3, stride=3, padding=0)
